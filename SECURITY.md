...
- Every tool call stats `token.json` (mtime, length and inode) in both modes. A changed or vanished file from a
  ...
  picks up the live grant without a restart. A
  `login` file (`obtained_by: "device_code"`) or a changed scope is a replacement
  and resets state. A transient refresh failure, including a non-deleting Entra refusal, is retried at
...
| Put task content in the `serve` log | No line the server logs carries a task title, body, category, checklist item or list name. A list appears in logs only as an opaque `lst_…` hash from `cache::log_ref`, and the log-hygiene test in `tests/tools_read.rs` re-runs its own binary to read the real stderr. See [Known gaps](#known-gaps) for the one exception that remains. |
| Put Microsoft's `error_description` in the `serve` log | When Microsoft refuses the boot refresh, `serve` logs **one** line, built by `auth::EntraFailure::log_error`: the AADSTS code (or the OAuth `error` string when there is none), this project's own summary and remediation from `auth/aadsts.rs`, and the Trace ID and Correlation ID that Microsoft support asks for. Microsoft's `error_description` is dropped from that line because its sign-in and Conditional Access messages commonly quote the account name, and the compose `json-file` driver persists stderr through every restart of the loop that `restart: unless-stopped` turns a refusal into. An unrecognised code stays diagnosable by its code and ids. It is one line even when the refusal deletes a dead `token.json` (AADSTS 530036, 70008, 700082): the boot refresh goes through `TokenProvider::boot_token`, which does not log the deletion separately, because the remediation in that line already says `token.json was deleted`; a runtime refresh keeps the `deleted token.json` warning, since a refused tool call is logged by nothing else. The description still reaches the places meant for the person who asked: `login` and `doctor` print it to stdout, and a tool call refused at runtime returns it in its `auth_failed` result to the MCP client. Unit tests in `src/auth/mod.rs` and `src/cli/commands.rs` assert the logged line from a description that names an account, and a re-exec pair in `tests/token_store.rs` reads the real stderr of a dead-token refusal on both paths. |
