...
**195 tests per run: 111 unit** (in the lib; `main.rs` has none) **and 84
integration** — cli_smoke 10, graph_client 10, http_auth 2, shutdown 6 (one is the
`child_process_entry` body, a no-op outside the child), token_store 14, tools_read 19,
...
`tools/`, `server.rs`, `mcp.rs`, `http.rs` and the six subcommands exist. The 195
