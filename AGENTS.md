...
**223 tests per run: 115 unit** (in the lib; `main.rs` has none) **and 108
...
`tools/`, `server.rs`, `mcp.rs`, `http.rs` and the six subcommands exist. The 223
...
- **`serve` can optionally start before sign-in.** With
  ...
  only when `rotated_from` names the prior file;
